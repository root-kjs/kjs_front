...
let bottom = Number(prompt( "1. 삼각형 밑변:" ));
let height = Number(prompt( "1. 삼각형 높이:" ));
let extent = (bottom * height) / 2;

// console.log( `삼각형 넓이: ${ extent } ` );

// 문제 2: 섭씨를 화씨로 변환
// 섭씨 온도를 prompt 함수로 입력받아, 화씨 온도로 변환하여 콘솔에 출력하는 프로그램을 작성하시오.
// 화씨 변환 공식: (섭씨 온도 * 9 / 5) + 32

let temp = prompt("섭씨 온도");
let fTemp = ( ( Number(temp) * 9 ) / 5) + 32;

console.log( ` 화씨온도 : ${ fTemp } ` );

// 문제 3: 나이 계산기
// 태어난 년도를 prompt 함수로 입력받아, 2025년 기준 현재 나이를 계산하여 콘솔에 출력하는 프로그램을 작성하시오.
// 나이 계산 공식: 2025 - 태어난 년도

let birth = prompt("태어난 년도") * 1;
const year = 2025; 
let age = year - birth;
...
let heightC = parseFloat( prompt("키(cm)") );
let weight = parseFloat( prompt("몸무게(kg)") );
// console.log( heightC, weight );

let heightM =  heightC / 100 ;
console.log( heightM );
...
let rating = score >= 90 ? "A" : score >= 80 && score <= 90 ? "B" : score < 70 ? "C" : "(강사님 70~80점 사이 등급이 없어요) 1"
console.log( ` 점수: ${ score } // ${ rating } 등급 ` );
...
let ageYouth = Number( prompt( "나이" ) ) ;
// let youth = age >= 20 && age <= 29
let eventTarget = ageYouth >= 20 && ageYouth <= 29 ? "이벤트 대상입니다." : "이벤트 대상이 아닙니다.";

console.log( ` 당신(${ ageYouth })은 ${ eventTarget } ` );
...
let likeCount = 3 ;
let likeCountAdd = ++likeCount

console.log( ` 좋아요: [ ${ likeCountAdd } ]` );
