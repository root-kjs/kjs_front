...
-----------------------------------------------------------------------------------------*/
const 전화번호부 = [
    { 전화번호코드 : 1, 이름 : '유관순', 전화번호 : '010-1234-5678', 메모 : '독립운동가' },
    { 전화번호코드 : 2, 이름 : '이순신', 전화번호 : '010-9101-1121', 메모 : '장군' }
];

// 1. 등록함수 구현로직
function 등록함수(){
    console.log( '<----------- 등록함수 exe ---------->' )
    let 자동코드 = 전화번호부.length;
    const name = document.querySelector('#nameInput').value;   console.log( name )
    const call = document.querySelector('#callInput').value;   console.log( call )
    const memo = document.querySelector('#memoInput').value;   console.log( memo )

    자동코드++
    const 전화번호 = { 전화번호코드 : 자동코드 , 이름 : name , 전화번호 : call , 메모 : memo };
    전화번호부.push( 전화번호 ); console.log( 전화번호부 )
...
function 조회함수(){
    ...
    content.innerHTML = html;  console.log( html )
    document.querySelector('#sum').innerHTML = 전화번호부.length;
} // func end
