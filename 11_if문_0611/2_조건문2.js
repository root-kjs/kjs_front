...
let seatList = [ 'O', 'X', 'O' ]; // ('O': 예약 완료, 'X': 예약 가능)
let seat = prompt( "좌석번호" ); // 사용자로부터 예약할 좌석의 인덱스 번호(0, 1, 2)를 입력받아
...
let gamePoint = Number( prompt('점수') ); 
...
let agePlayer = Number(prompt('나이:')); 

if( agePlayer < 8 ){ 
    console.log( '무료' );
}else if( agePlayer <= 19  ){
    console.log( '5,000원' );
}else if( agePlayer < 65  ){
    console.log( '10,000원' );
}else{
    console.log( '3,000원' );
}
...
// 없는 번호 : 범위 논리(0~2) --> 논리 연산자 사용해야 함!

let drinkNames = ['콜라', '사이다', '커피'];
let drinkPrices = [1000, 1000, 1500];
let drinkNum = Number(prompt('음료번호(0~2):')); 

if( drinkNum >= 0 && drinkNum <= 2 ){ 
    console.log( ` 선택하신 음료는 ${ drinkNames[drinkNum] }입니다. 가격은 ${ drinkPrices[drinkNum] }원입니다. ` );
