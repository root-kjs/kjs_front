...
// // '2' 또는 '4'이면 'red'
// //console.log( idNum.length ); // 숫자형은 길이가 없다. length 길이가 없음. 문자나 배열만 해당함.
// //console.log( idNum[6] ); 
...
if( amount >= 50000 ){
    console.log( `최종 결제 금액: ${ amount - (amount * 10)/100 }원` );
}else if( amount >= 30000 ){
    console.log( `최종 결제 금액: ${ amount - (amount * 5)/100 }원` );
}else if( amount >= 10000 ){
    console.log( `최종 결제 금액: ${ amount - (amount * 1)/100  }원`);
}else{
    console.log( `최종 결제 금액: ${ amount  }원`);
}
...
let inputYear = Number( prompt(" 연도(year)를 입력: ") ) // 숫자값
let leapYear = (( inputYear % 4 === 0 ) && ( inputYear % 100 !== 0 )) || ( inputYear % 400 === 0 ) // 논리값(조건)
...
// [1] > [2] 비교
// (3) 배열 값의 위치(정렬)을 바꾼다. 
// (3) 이 때 저장된 배열의 값을 스왑(교환)처리하여(temp)라는 임시 변수를 활용한다.
...
let car = prompt( '차량정보 ');
//console.log( locationArray[ carArray.indexOf( car ) ] );
let searchCar = locationArray[ carArray.indexOf( car ) ];

if( carArray.indexOf( car ) != -1 ){
    console.log( ` ${ car } 차량위치: ${ searchCar } ` )
}else{
...
let courseList = ['수학', '영어', '과학', '국어'];
let cancelCourse = prompt( '수강취소 과목명 :' );
//console.log( courseList.splice(courseList.indexOf(exceptCourse),1) )
let index = courseList.indexOf(cancelCourse);
