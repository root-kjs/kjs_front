...
for( let index = 0; index <= numbers2.length -1; index++ ){
    if( newNumber.indexOf(numbers2[index]) === -1 ){
            console.log( newNumber.push(numbers2[index]) );
    }
}
...
let numbers1 = [5, 3, 4, 1, 2];

for( index = 0; index <= numbers1.length -1; index++ ){
    for( j = 0; j <= numbers.length -1; j++ ){
        if( numbers1[index] < numbers1[j]  ){
            let temp;
            temp = numbers1[index]
            numbers1[index] = numbers1[j]
            numbers1[j] = temp
        }
